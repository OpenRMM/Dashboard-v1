<?php
	...
	//Loop Trough Tasks
	$query2 = "SELECT * FROM tasks WHERE active='1' ORDER BY ID ASC";
	$tasks = mysqli_query($db, $query2);
	while($task = mysqli_fetch_assoc($tasks)){


		//Loop Trough PCs
		$query = "SELECT * FROM computerdata WHERE active='1'  ORDER BY ID ASC";
		$results = mysqli_query($db, $query);
		while($result = mysqli_fetch_assoc($results)){
			$json = getComputerData($results['ID'], array("*"), "latest");
			$alerts = $json['Alerts'];

			function parse($type){
				global $result, $json;
				if($result['condition1Type']=="Total Ram/Memory"){ 
					return $json['WMI_ComputerSystem']['Response'][0]['FreePhysicalMemory'];
				}
			}
			...
			switch($task['condition1Comparison']){
				case "=":
					$type=parse("true");
					if($type==$task['condition1Value']){  }
				break;
				case "!=":
					$type=parse("true");
					if($type!=$task['condition1Value']){  }
		...
		$query = "UPDATE tasks SET last_run='".$date."' WHERE ID='".$task['ID']."'";
		$results = mysqli_query($db, $query);
	}
